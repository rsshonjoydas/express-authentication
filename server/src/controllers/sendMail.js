import { google } from "googleapis";
import nodemailer from "nodemailer";
import env from "../config";

const { OAuth2 } = google.auth;
const OAUTH_PLAYGROUND = "https://developers.google.com/oauthplayground";

const oauth2Client = new OAuth2(
  env.MAIL_SEND_CLIENT_ID,
  env.MAIL_SEND_CLIENT_SECRET,
  OAUTH_PLAYGROUND,
  env.MAIL_REFRESH_TOKEN
);

const sendMail = (to, url, txt) => {
  oauth2Client.setCredentials({
    refresh_token: env.MAIL_REFRESH_TOKEN,
  });

  const accessToken = oauth2Client.getAccessToken();
  const smtpTransport = nodemailer.createTransport({
    service: "gmail",
    auth: {
      type: "OAuth2",
      user: env.EMAIL_ADDRESS,
      clientId: env.MAIL_SEND_CLIENT_ID,
      clientSecret: env.MAIL_SEND_CLIENT_SECRET,
      refreshToken: env.MAIL_REFRESH_TOKEN,
      accessToken,
    },
  });

  const mailOptions = {
    from: env.EMAIL_ADDRESS,
    to,
    subject: "RS Code",
    html: `
            <div style="max-width: 700px; margin:auto; border: 10px solid #ddd; padding: 50px 20px; font-size: 110%;">
            <h2 style="text-align: center; text-transform: uppercase;color: teal;">Welcome to the RS Code.</h2>
            <p>Congratulations! You're almost set to start using RS✮SHOP.
                Just click the button below to validate your email address.
            </p>
            
            <a href=${url} style="background: crimson; text-decoration: none; color: white; padding: 10px 20px; margin: 10px 0; display: inline-block;">${txt}</a>
        
            <p>If the button doesn't work for any reason, you can also click on the link below:</p>
        
            <div>${url}</div>
            </div>
        `,
  };

  smtpTransport.sendMail(mailOptions, (err, info) => {
    if (err) return err;
    return info;
  });
};

export default sendMail;
